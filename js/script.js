...
function QuantityChange(current_node) {
  ...
  document.getElementById("flash_text").style.animation = "none";
...
function AddToCart() {
  ...
  var content_table = document.getElementById("content");
  var num_rows = content_table.rows.length;
  var val =content_table.rows[num_rows-1].cells[2].innerHTML;
  if(parseFloat(val) <= 0) { window.alert("Please choose items"); return false };
...
function PlaceOrder() {
  ...
  if(customer_phone=="")
  {
    ...
    var phoneno = /^\d{10}$/;
    if(!customer_phone.match(phoneno)){
  ...
  console.log(JSON.stringify(cart_table_data)); 
    ...
    $.post("order.php", { 
          ...
          data:cart_table_data,
            ...
            $("#order-status").html(data);
            document.getElementById("orderplaced").value = 1;
...
function GoToHome() {
  ...
  if( document.getElementById("orderplaced").value ==1){
