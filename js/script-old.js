function CalculateTotal() {
  ...
  for (var i = 1; i < num_rows - 3; i++) {
    if(content_table.rows[i].cells.length != 7)
      continue;
    qty_cell = content_table.rows[i].cells[5].childNodes[0].value;
...
function QuantityChange(current_node) {
  ...
  var per_rate = parseFloat(childnodes[childnodes.length-8].innerHTML);
...
function Checkout() {
  ...
  console.log(cart_table_json);
   ...
   /* var table = $('#table').tableToJSON();
    console.log(table);
    document.getElementById("demo").innerHTML =JSON.stringify(table);
...
function PlaceOrder() {
 ...
 /*  $.post("order.php", { 
  ...
  var cart_table = document.getElementById("cart-table");
  var num_cart_rows = cart_table.rows.length;
  /* console.log(parseFloat(cart_table.rows[num_cart_rows-3].cells[2].innerHTML ));
  console.log(parseFloat(cart_table.rows[num_cart_rows-2].cells[1].innerHTML ));
  console.log(parseFloat(cart_table.rows[num_cart_rows-1].cells[1].innerHTML ));
 */
